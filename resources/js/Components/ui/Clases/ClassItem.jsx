...
const ClassItem = ({ classData, isOpen, onToggle, onEdit, onDelete }) => {
    ...
    const handleAddStudent = () => {
        fetch(`/api/classes/${classData.id}/students`, {
            ...
            .then(addedStudent => {
                // Inicializa classData.estudiantes si está indefinido
                if (!classData.estudiantes) {
                    classData.estudiantes = [];
                }

                // Añade el nuevo estudiante
    ...
    const handleStudentDelete = (studentId) => {
        fetch(`/api/classes/` + classData.id + `/students/${studentId}`, {
            method: 'DELETE',
        })
            .then(response => {
                if (!response.ok) {
...

            {
                isAddingStudent && (
                    ...
                    >
                        ...
                        {/* Contenido del modal */}
                        <motion.div
                            className="relative z-10 bg-white rounded-lg p-6 w-full max-w-md mx-auto shadow-lg"
                        >
